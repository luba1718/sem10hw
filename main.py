...
def getToken():
    token = ''
    if os.path.isfile(g.BOT_TOKEN_FILENAME):
        f = open(g.BOT_TOKEN_FILENAME, "r")
        token = f.read()
        f.close()
    else:
...
def game(callBackData):
    ...
    if not buttonNumber == 9: 
        ...
        charList.pop(0)  
        if charList[buttonNumber] == g.SYMBOL_UNDEF:  
            charList[buttonNumber] = g.SYMBOL_X  
            if isWin(charList, g.SYMBOL_X):  
                message = g.ANSW_YOU_WIN
            else:  
                if countUndefinedCells(charList) != 0: 
                    isCycleContinue = True
                    while (isCycleContinue):
                        rand = random.randint(0, 8)  
                        if charList[rand] == g.SYMBOL_UNDEF:  
                            charList[rand] = g.SYMBOL_O
                            isCycleContinue = False  
                            if isWin(charList, g.SYMBOL_O):  
                                message = g.ANSW_BOT_WIN
        else:
            alert = g.ALERT_CANNOT_MOVE_TO_THIS_CELL

        if countUndefinedCells(charList) == 0 and message == g.ANSW_YOUR_TURN:
            message = g.ANSW_DRAW
    ...
    if message == g.ANSW_YOU_WIN or message == g.ANSW_BOT_WIN or message == g.ANSW_DRAW:
        message += '\n'
        for i in range(0, 3):
            message += '\n | '
            for j in range(0, 3):
...
def newGame(update, _):
    ...
    update.message.reply_text(g.ANSW_YOUR_TURN, reply_markup=InlineKeyboardMarkup(getKeyboard(data)))
...
def help_command(update, _):
    update.message.reply_text(g.ANSW_HELP)
